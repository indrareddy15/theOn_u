...
import {
    ...
    LayoutDashboard, Package, Users, Tag, LogOut, Settings,
    ShoppingBag, Percent, Megaphone, Box, MessageSquare
} from "lucide-react";
...
export default function Layout({ children, currentPageName }) {
    ...
    const isAdminPage = location.pathname.includes('/admin') || currentPageName?.startsWith('Admin');
    ...
    }, [location.pathname, openLoginModal, user?.email, isAdminPage, loadWishlistCount, loadCartCount, showSearchBar]);
    ...
    const loadCategories = async () => {
        try {
            const cats = await base44.entities.Category.filter({ is_active: true }, 'display_order');
            setCategories(cats);
        } catch (error) {
    ...
    const loadDisclaimers = async () => {
        try {
            const discs = await base44.entities.Disclaimer.filter({ is_active: true, position: 'footer' }, 'display_order');
    ...
    const handleSearch = (e) => {
        e.preventDefault();
        if (searchQuery.trim()) {
            navigate(createPageUrl(`Shop?q=${encodeURIComponent(searchQuery.trim())}`));
            setSearchQuery('');
            setShowSearchBar(false);
                    ...
                    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        <div className="flex justify-between items-center h-16">
                            <Link to={createPageUrl('AdminDashboard')} className="flex items-center gap-2">
                                ...
                                <div>
                                    <div className="font-bold text-gray-900">TheOn</div>
                            ...
                            <nav className="hidden md:flex items-center gap-6">
                                <Link
                                    ...
                                    className={`flex items-center gap-2 text-sm font-medium transition-colors ${currentPageName === 'AdminDashboard' ? 'text-gray-900' : 'text-gray-600 hover:text-gray-900'
                                        }`}
                                >
                                ...
                                <Link
                                    ...
                                    className={`flex items-center gap-2 text-sm font-medium transition-colors ${currentPageName === 'AdminProducts' || currentPageName === 'AdminProductForm' ? 'text-gray-900' : 'text-gray-600 hover:text-gray-900'
                                        }`}
                                >
                                ...
                                <Link
                                    ...
                                    className={`flex items-center gap-2 text-sm font-medium transition-colors ${currentPageName === 'AdminOrders' ? 'text-gray-900' : 'text-gray-600 hover:text-gray-900'
                                        }`}
                                >
                                ...
                                <Link
                                    ...
                                    className={`flex items-center gap-2 text-sm font-medium transition-colors ${currentPageName === 'AdminInventory' ? 'text-gray-900' : 'text-gray-600 hover:text-gray-900'
                                        }`}
                                >
                                ...
                                <Link
                                    ...
                                    className={`flex items-center gap-2 text-sm font-medium transition-colors ${currentPageName === 'AdminCustomers' ? 'text-gray-900' : 'text-gray-600 hover:text-gray-900'
                                        }`}
                                >
                                ...
                                <Link
                                    ...
                                    className={`flex items-center gap-2 text-sm font-medium transition-colors ${currentPageName === 'AdminSupport' ? 'text-gray-900' : 'text-gray-600 hover:text-gray-900'
                                        }`}
                                >
                                ...
                                <Link
                                    ...
                                    className={`flex items-center gap-2 text-sm font-medium transition-colors ${currentPageName === 'AdminContentManagement' ? 'text-gray-900' : 'text-gray-600 hover:text-gray-900'
                                        }`}
                                >
                                ...
                                <Link
                                    ...
                                    className={`flex items-center gap-2 text-sm font-medium transition-colors ${currentPageName === 'AdminSettings' ? 'text-gray-900' : 'text-gray-600 hover:text-gray-900'
                                        }`}
                                >
                            ...
                            <nav className="hidden lg:flex items-center gap-8">
                                ...
                                {categories.slice(0, 3).map(cat => (
                                    <Link
                                        ...
                                        className="text-sm font-medium text-gray-700 hover:text-gray-900 transition-colors capitalize"
                                    >
                                        {cat.name}
                                    </Link>
                                ))}
                                ...
                                {user ? (
                                    <>
                                        ...
                                        >
                                            ...
                                            {cartCount > 0 && (
                                                <Badge className="absolute -top-1 -right-1 h-5 w-5 flex items-center justify-center p-0 bg-gray-900">
                                                    {cartCount}
                                                </Badge>
                                            )}
                                        </Button>
                                        <DropdownMenu>
                                            ...
                                            <DropdownMenuContent align="end" className="w-48">
                                                ...
                                                <DropdownMenuItem onClick={() => navigate(createPageUrl('Orders'))}>
                        ...
                        <nav className="max-w-7xl mx-auto px-4 py-4 space-y-1">
                            ...
                            {categories.map(cat => (
                                <Link
                                    key={cat.id}
                                    to={createPageUrl(`Shop?category=${cat.slug}`)}
                                    className="block px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 rounded-lg capitalize"
                                    onClick={() => setMobileMenuOpen(false)}
                                >
                                    {cat.name}
                                </Link>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                {disclaimers.map(disclaimer => (
                                    <div key={disclaimer.id} className="flex items-start gap-3">
                                        <span className="text-2xl">{getDisclaimerIcon(disclaimer.icon)}</span>
                                        <div>
                                            <h4 className="font-semibold text-gray-900 mb-1">{disclaimer.title}</h4>
                                            <p className="text-sm text-gray-600">{disclaimer.content}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
                        <div className="col-span-1 md:col-span-2">
                            ...
                            <p className="text-gray-600 text-sm mb-4 max-w-md">
                                Your destination for premium fashion and style. Discover the latest trends in clothing and accessories.
                            </p>
                        </div>
                        ...
                        <div>
                            ...
                            <ul className="space-y-2 text-sm text-gray-600">
                                <li><Link to={createPageUrl('Shop')} className="hover:text-gray-900">Shop</Link></li>
                                <li><Link to={createPageUrl('Orders')} className="hover:text-gray-900">Orders</Link></li>
                                <li><Link to={createPageUrl('Profile')} className="hover:text-gray-900">Profile</Link></li>
                            </ul>
                        </div>
