...
import { Heart, ShoppingCart, Star, Truck, Shield, RefreshCw, Minus, Plus, Ruler } from "lucide-react";
...
export default function ProductDetail() {
    ...
    const loadSizeChart = useCallback(async (category) => {
        try {
            const charts = await base44.entities.SizeChart.filter({ category, is_active: true });
            if (charts.length > 0) {
    ...
    const loadRelatedProducts = useCallback(async (category) => {
        try {
            const data = await base44.entities.Product.filter({ category, is_active: true }, '-created_date', 8);
    ...
    const loadProduct = useCallback(async () => {
        try {
            setLoading(true);
            console.log('Loading product with ID:', productId, 'parsed:', parseInt(productId));
    ...
    const loadReviews = useCallback(async () => {
        try {
            const reviews = await base44.entities.CustomerReview.filter({ product_id: parseInt(productId) });
            setReviews(reviews);
        } catch {
    ...
    const handleSubmitReview = async () => {
        ...
        try {
            ...
            const currentReviews = await base44.entities.Review.filter({ product_id: parseInt(productId), is_approved: true });
    ...
    const images = product.images?.length > 0 ? product.images : ['https://images.unsplash.com/photo-1523381210434-271e8be1f52b?w=800'];
                        ...
                        <div className="grid grid-cols-4 gap-4">
                            {images.map((img, idx) => (
                                <button
                                    ...
                                    className={`aspect-square rounded-lg overflow-hidden border-2 transition-colors ${selectedImage === idx ? 'border-gray-900' : 'border-gray-200 hover:border-gray-400'
                                        }`}
                                >
                                ...
                                <div className="flex gap-3">
                                    {product.colors.map((color, idx) => (
                                        <button
                                            ...
                                            className={`w-12 h-12 rounded-full border-2 transition-colors ${selectedColor?.name === color.name ? 'border-gray-900 scale-110' : 'border-gray-200'
                                                }`}
                                    ...
                                    {sizeChart && (
                                        <Dialog open={sizeChartOpen} onOpenChange={setSizeChartOpen}>
                                            ...
                                            <DialogContent className="max-w-2xl">
                                                <DialogHeader>
                                                    <DialogTitle>Size Chart - {sizeChart.category}</DialogTitle>
                                                </DialogHeader>
                                                <div className="mt-4">
                                                    <Table>
                                                        <TableHeader>
                                                            <TableRow>
                                                                ...
                                                                {sizeChart.size_data[0]?.chest && <TableHead>Chest ({sizeChart.unit})</TableHead>}
                                                                {sizeChart.size_data[0]?.waist && <TableHead>Waist ({sizeChart.unit})</TableHead>}
                                                                {sizeChart.size_data[0]?.length && <TableHead>Length ({sizeChart.unit})</TableHead>}
                                                                {sizeChart.size_data[0]?.shoulder && <TableHead>Shoulder ({sizeChart.unit})</TableHead>}
                                                            </TableRow>
                                                        </TableHeader>
                                                        <TableBody>
                                                            {sizeChart.size_data.map((row, idx) => (
                                                                <TableRow key={idx}>
                                                                    ...
                                                                    {row.shoulder && <TableCell>{row.shoulder}</TableCell>}
                                                                </TableRow>
                                                            ))}
                                                        </TableBody>
                                                    </Table>
                                ...
                                <div className="flex gap-2">
                                    {product.sizes.map((size, idx) => (
                                        <button
                                            ...
                                            className={`px-6 py-3 border-2 rounded-lg font-medium transition-colors ${selectedSize === size.size
                    ...
                    <Tabs defaultValue="reviews" className="w-full">
                        ...
                        <TabsContent value="details" className="mt-8">
                            <div className="prose max-w-none">
                                ...
                                {product.specification && <p className="text-gray-600">{product.specification}</p>}
                                {product.material && (
                                    <div className="mt-4">
                                    ...
                                    {reviews.map(review => (
                                        <div key={review.id} className="border-b pb-6">
                                            ...
                                            <p className="text-xs text-gray-500 mt-2">
                                                {review.created_date && isValid(new Date(review.created_date)) ?
