                ...
                <div className="space-y-3">
                    ...
                    {chats.filter(c => c.status !== 'resolved').length === 0 ? (
                        ...
                        chats.filter(c => c.status !== 'resolved').map(chat => (
                            <Card
                                key={chat.id}
                                className={`cursor-pointer transition-colors ${selectedChat?.id === chat.id ? 'border-gray-900' : 'hover:border-gray-400'
                                    }`}
                            ...
                            >
                                <CardContent className="p-4">
                                    <div className="flex justify-between items-start mb-2">
                                        <div>
                                            <p className="font-medium text-gray-900">{chat.user_name || 'Unknown User'}</p>
                                            <p className="text-xs text-gray-500">{chat.user_email || 'No email'}</p>
                                        </div>
                                        <Badge className={getStatusColor(chat.status)}>
                                            {chat.status ? chat.status.replace('_', ' ') : 'Unknown'}
                                        </Badge>
                                    </div>
                                    <p className="text-sm text-gray-600 truncate">{chat.last_message || 'No messages yet'}</p>
                                    <div className="flex justify-between items-center mt-2">
                                        <span className="text-xs text-gray-500">
                                            {chat.updated_date ? format(new Date(chat.updated_date), 'MMM dd, HH:mm') : 'Unknown date'}
                                        </span>
