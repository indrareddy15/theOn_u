...
import {
    ...
    Eye, Package, AlertCircle, Download, CheckSquare, Square
} from "lucide-react";
import {
...
export default function AdminProducts() {
    ...
    const applyFilters = () => {
        ...
        if (statusFilter !== 'all') {
            filtered = filtered.filter(p => {
                ...
                if (statusFilter === 'low_stock') return (p.total_stock || 0) > 0 && (p.total_stock || 0) <= 10;
                return true;
            });
        }
    ...
    // Export Functions
    const exportToCSV = () => {
        const headers = ['Title', 'SKU', 'Category', 'Price', 'Sale Price', 'Stock', 'Status'];
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8 gap-4">
                ...
                <div className="flex gap-2">
                    ...
                    <Button onClick={() => navigate(createPageUrl('AdminProductForm'))}>
                        ...
                        <TableHeader>
                            <TableRow>
                                ...
                                <TableHead
                                    ...
                                    sortDirection={sortConfig.key === 'total_stock' ? sortConfig.direction : null}
                                >
                                    Stock
                                </TableHead>
                                            ...
                                            <div className="flex items-center gap-2">
                                                {(product.total_stock || 0) === 0 ? (
                                                    ...
                                                    <>
                                                        ...
                                                        <span className="text-yellow-600 font-medium">{product.total_stock}</span>
                                                    </>
                                                ) : (
                                                    <span className="text-gray-900">{product.total_stock}</span>
                                                )}
                                            </div>
                                        </TableCell>
                                        <TableCell>
                                        ...
                                        <TableCell className="text-right">
                                            <DropdownMenu>
                                                ...
                                                <DropdownMenuContent align="end">
                                                    ...
                                                    <DropdownMenuItem onClick={() => navigate(createPageUrl(`AdminProductForm?id=${product.id}`))}>
            ...
            <AlertDialog open={bulkActionDialog.open} onOpenChange={(open) => setBulkActionDialog({ open, action: null })}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        <AlertDialogTitle>Confirm Bulk Action</AlertDialogTitle>
                        <AlertDialogDescription>
                            Are you sure you want to {bulkActionDialog.action} {selectedProducts.size} product(s)?
